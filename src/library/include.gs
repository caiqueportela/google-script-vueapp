...
var Include = (function (ns) {
  ...
  const addScript = (text) => (text.match(/^\s*?\<script\>/i) ? text : `<script>\n${text}</script>\n`);

  // this'll add style tags if they are not there
  const addStyle = (text) => (text.match(/^\s*?\<style\>/i) ? text : `<style>\n${text}</style>\n`);
   ...
   */
  ns.gs = (scripts, library) => addScript(scripts.map((d) => ScriptApp.getResource(d).getDataAsString()).join('\n\n'));
   ...
   */
  ns.html = (scripts, ext) =>
    scripts.map((d) => HtmlService.createHtmlOutputFromFile(d + (ext || '')).getContent()).join('\n\n');
  ...
  ns.mjs = ({ sources, initialize = true, promiseName = 'moduleImports' }) => {
    ...
    const reducer = sources
      .map((f, i) => `    ${f.key}: modules[${i}]${f.exportName ? `.${f.exportName}` : ''}`)
      .join(',\n');
